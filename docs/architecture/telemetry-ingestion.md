...
All timestamps require offsets; comparison uses instants and canonical key encoding uses
UTC. Observation cannot precede start, and resolution must fall between start and
...
Source projections select the greatest tuple `(observed_at, resolved_rank, content_digest)`.
Resolution wins an equal-time tie; equal-state conflicts have a stable digest tiebreaker.
...
Policy `service-category-window/1` uses no score or LLM. A candidate must have the same
...
One candidate joins. No candidate creates. Multiple candidates create a separate incident
with `multiple_matches`; there is no arbitrary tie winner or automatic merge. The receipt
...
as scope rules. In-scope candidates record both positive and negative decisions, including
environment/service/category mismatch and terminal state. A row lock refreshes a selected
incident before attachment, preventing joining an incident that concurrently terminated.

At most 256 candidate anchors are inspected. Exceeding the bound fails the transaction
with `candidate_limit_retry_after_review`; no truncation is presented as a complete decision.
An operator must review the storm or policy before redelivery. Prior decisions are never
overwritten when policy code changes. Different services stay separate in v1. Declared
...
Creation records `incident.opened` with initial title, severity, state, environment,
correlation anchor and investigation-request flag. `alert.received` and `alert.normalised`
...
Existing occurrence updates keep their attachment even if its incident has terminated;
they do not reopen it. New occurrences cannot correlate to terminal incidents. Automated
reopen, detach and merge are deferred. The dispatcher enters the existing Phase 4 kernel;
...
Dispatch failure records `trigger_failed` and propagates the exception. If creation never
...
Every ingestion transaction uses explicit tenant binding, READ COMMITTED and a PostgreSQL
transaction advisory lock derived from the tenant UUID. This serializes duplicates,
cross-fingerprint correlation and firing/resolution updates across connections/processes.
Hash collision can over-serialize tenants but cannot merge their data. Statement timeouts
bound database waits. Database errors roll back the receipt, projection, events and request.
...
Stage counters and duration histograms use bounded stage/outcome labels, not tenant IDs
...
`tests/ingestion` exercises canonical/source-format validation, malicious data, A-J incident
scenarios, real application-role commits, duplicate races, resolution races, explanations,
