...
for at most 600 seconds from the last successful fetch then fail closed; a duplicate `kid` makes
the set ambiguous and refuses it). Normal refresh bounds revocation lag to the 300-second TTL;
issuer outage can extend it to that 600-second stale bound.
...
registry and checks the live schema: RLS enabled and forced, exactly one canonical permissive
`ALL TO PUBLIC` policy with USING and explicit WITH CHECK equal to
`tenant_id = app.current_tenant_id()`, `tenant_id` present and not null, every table classified,
every model-declared foreign key between tenant tables present with its exact signature, actions
and referenced uniqueness, the application role neither privileged nor an
...
index sources, range agreement, complete runtime-to-dev lock inclusion at identical versions, a
reasoned prerelease allowlist and frontend integrity digests.
